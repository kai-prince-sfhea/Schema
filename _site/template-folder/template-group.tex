...
\begin{definition}[Group]\protect\hypertarget{def-group}{}\label{def-group}
...
\begin{itemize}
\item
  For all \({g},{h},{j}\) in \(\Group\), one has
  \(\GroupOperation{(\GroupOperation{g}{h})}{j}=\GroupOperation{g}{(\GroupOperation{h}{j})}\).
\item
  There exists an element \(\Identity\) in \(\Group\) such that, for
  every \({g}\) in \(\Group\), one has
  \(\GroupOperation{\Identity}{g}={g}\) and
  \(\GroupOperation{g}{\Identity}={g}\). Such an element is unique and
  is called the \textbf{identity element}.
\item
  For each \({g}\) in \(\Group\), there exists an element \({h}\) in
  \(\Group\) such that \(\GroupOperation{g}{h}=\Identity\) and
  \(\GroupOperation{h}{g}=\Identity\), where \(\Identity\) is the
  identity element.

  For each \({g}\), the element \({h}\) is unique and is called the
  \textbf{inverse} of \({h}\) and is denoted \(\Inverse{g}\).
\end{itemize}
